...
|--------|----------|------------------------|
| **Code-first** | Аннотации в коде → `swag init` → генерация `docs/` | ✅ |
| **Schema-first** | OpenAPI YAML/JSON → `oapi-codegen` → генерация кода | ❌ |
| **Embedded Spec** | Ручное написание спецификации в коде | ❌ |
...
> ⚠️ **Вставьте сюда скриншот работающей страницы Swagger UI**
...
| URL | Описание |
|-----|----------|
| http://localhost:8080/health | Healthcheck (возвращает `OK`) |
| http://localhost:8080/docs/ | Swagger UI — интерактивная документация |
| http://localhost:8080/docs/doc.json | OpenAPI спецификация в формате JSON |
| http://localhost:8080/api/v1/notes | API заметок |
...
1. **CI/CD интеграция**
   - Добавить проверку актуальности `docs/` в CI
   - Автоматическая регенерация при изменении аннотаций

2. **Валидация**
   - Добавить `binding:"required"` теги для автоматической валидации
   - Использовать `go-playground/validator`

3. **OpenAPI 3.0**
   - swag поддерживает только OpenAPI 2.0 (Swagger)
   - Для OpenAPI 3.0 можно использовать `ogen` или `oapi-codegen`
...
```bash
# Регенерировать документацию после изменения аннотаций
swag init -g cmd/api/main.go -o docs

# Проверить форматирование swag
swag fmt
